...
const DrawLayer = () => {
    ...
    // 활성화 된 지도 기능
    const gisMode = useSelector((state: RootState) => state.view.sslGis.gisFuncMode);
    ...
    const [circleTooltip, setCircleTooltip] = useState<string | null>();
    ...
    useEffect(() => {
        if (gisMode === GIS_FUNC_MODE.N) {
            map.eachLayer(layer => {
                if (layer.options.pane === "drawLayer") {
        ...
        if (isDrawing) {
            if (gisMode === GIS_FUNC_MODE.L) {
                if (event.originalEvent.key === "Escape") { // ESC 키를 누를 경우
                    if (linePositions.length < 3) { // 아직 drawing 중일 경우
    ...
    useEffect(() => {
        if (polygonRef.current) {
            polygonRef.current.leafletElement.bringToFront(); // 레이어를 맨 앞으로 이동
        }
    }, [polygonRef]);
